  ...
  const handleSubmit = (e) => {
    ...
    axios.post('http://localhost:5000/api/login', loginData)
      .then((response) => {
        ...
        const normalizedRole = role === 'businessowner' ? 'business_owner' : role;

        // Use login function from AuthContext to set the role
        ...
        Cookies.set('userRole', normalizedRole);

        // Redirect based on role
        if (normalizedRole === 'business_owner') {
          navigate('/business_owner_dashboard');  // Redirect to business owner dashboard
        } else if (normalizedRole === 'customer') {
          navigate('/customer_dashboard');  // Redirect to customer dashboard
        }
      })
      .catch((error) => {
        Swal.fire({
